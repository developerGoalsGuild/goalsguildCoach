...
async function runSimpleSchema() {
  ...
  try {
    ...
    // Primeiro, executar apenas CREATE TABLE
    console.log('📝 Criando tabelas...');
    for (const statement of statements) {
      if (statement.toUpperCase().startsWith('CREATE TABLE') || statement.toUpperCase().startsWith('INSERT INTO')) {
        try {
          await client.query(statement);
        } catch (error) {
          if (!error.message.includes('already exists')) {
            console.error('Erro em statement:', statement.substring(0, 100));
            throw error;
          }
        }
      }
